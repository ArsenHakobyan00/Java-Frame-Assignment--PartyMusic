...
/**
 * Description: This progam displays a number of songs from a specific text
 * (.txt) file.
 * 
...
public class PartyMusicFrame extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		int tapeLength = 0;
		String fileName = "./ahakobyan_G10_A04/ahakobyan_G10_A04_Party_Music/";

		if (lengthField.getText().isEmpty()) { // if there's no tape length
			// Display an error message
			JOptionPane.showMessageDialog(resultDisplay, "Please enter the length of tape.", "No length",
					JOptionPane.ERROR_MESSAGE);
		}

		else if (fileField.getText().isEmpty()) { // if the file field is empty
			// Display a warning
			JOptionPane.showMessageDialog(resultDisplay, "File name is being set to \"tape.txt\"", "No file name",
		...
		else if (!fileField.getText().contains(".txt")) { // if the file name has no .txt extension
	...
	public void openFile(File fileName, int tapeLen) {
		...
		try {
			...
			while (fileScanner.hasNext()) {
				...
				try {
					...
					if (partyTape.addSong(minutes, seconds) == true)
						resultDisplay.append(String.format("%-30s%15d%10d%10d%10d%n", songName, minutes, seconds,
								partyTape.getMinutesUsed(), partyTape.getSecondsUsed()));
					else
			...
			resultDisplay.append("\nThere are " + partyTape.getMinutesLeft() + " minutes "
					+ partyTape.getSecondsLeft() % 60 + " seconds of tape left.");
		} // try block

		catch (FileNotFoundException e) {
			System.err.println(file.getName() + " not found");
			System.err.println("\tERROR CODE: -1");
			JOptionPane.showMessageDialog(resultDisplay, "Your file name " + file.getName() + " does not exist",
					"File Not Found",
					JOptionPane.ERROR_MESSAGE);
